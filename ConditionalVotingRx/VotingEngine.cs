...
namespace ConditionalVotingRx
{
  public class VotingEngine
  {
    ...
    public IObservable<VotingResult> VotingResults =>
      this.eventSeq
          .CombineLatest(
            this.Subscriptions.StartWith(ImmutableList<Subscription>.Empty),
    ...
    private IObservable<ICollection<Subscription>> Subscriptions =>
      this.subscriptionSeq.Scan(ImmutableList<Subscription>.Empty, (list, subscription) => list.Add(subscription));
